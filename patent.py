...
class Event:
  ...
  @Flow.From()
  def pull(storage:Storage, assignpath:str, codes:pandas.DataFrame):
...
class Classification:
  ...
  @Flow.From()
  def pull(storage:Storage, assignpath:str, codes:pandas.DataFrame, extended=False):
...
for h in flow.keys():
  ...
  flow[h]['patent-code'] = Code.pull(f0[h]['profiling'], assignpath=D[h]+'/assignement.yaml').map(D[h]+'/code.pkl')

  flow[h]['patent-geoloc'] = Geoloc.pull(f0[h]['profiling'], 
                                         assignpath=D[h]+'/assignement.yaml', 
                                         codes=flow[h]['patent-code'], 
                                         geodata=fg['Misc']['geodata'],).map(D[h]+'/geoloc.pkl')

  flow[h]['patent-event'] = Event.pull(f0[h]['profiling'], 
                                       assignpath=D[h]+'/assignement.yaml',
                                       codes=flow[h]['patent-code']).map(D[h]+'/event.pkl')

  flow[h]['patent-classify'] = Classification.pull(f0[h]['profiling'],
                                        assignpath=D[h]+'/assignement.yaml',
